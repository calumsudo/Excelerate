...
class DateEntry(ttk.Entry):
    ...
    def _on_b1_press(self, event):
        ...
        x, y = event.x, event.y
        if ("disabled" not in self.state()) and self.identify(
            x, y
        ...
        if self.focus_get() is not None:
            if self.focus_get() == self:
                ...
                if (
                    type(x) != int
                    or type(y) != int
                    or self.identify(x, y) != self._downarrow_name
                ):
                    self._top_cal.withdraw()
                    self.state(["!pressed"])
            else:
    ...
    def configure(self, cnf={}, **kw):
        ...
        if (
            entry_kw.get("state") == "readonly"
            and self._cursor == "xterm"
            and "cursor" not in entry_kw
        ):
