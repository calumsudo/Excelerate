...
class CTkFont(Font):
    ...
    def __init__(
        ...
        super().__init__(
            family=(
                ThemeManager.theme["CTkFont"]["family"] if family is None else family
            ),
            size=-abs(self._size),
            weight=(
                ThemeManager.theme["CTkFont"]["weight"] if weight is None else weight
            ),
            slant=slant,
            underline=underline,
    ...
    def create_scaled_tuple(self, font_scaling: float) -> Tuple[str, int, str]:
        ...
        return (
            self._family,
            round(-abs(self._size) * font_scaling),
            self._tuple_style_string,
        )
