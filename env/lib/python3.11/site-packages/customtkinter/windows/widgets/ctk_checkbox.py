...
class CTkCheckBox(CTkBaseClass):
    ...
    def configure(self, require_redraw=False, **kwargs):
        ...
        if "checkmark_color" in kwargs:
            self._checkmark_color = self._check_color_type(
                kwargs.pop("checkmark_color")
            )
            require_redraw = True
        ...
        if "variable" in kwargs:
            ...
            if self._variable is not None and self._variable != "":
                ...
                self._check_state = (
                    True if self._variable.get() == self._onvalue else False
                )
                require_redraw = True
    ...
    def toggle(self, event=0):
        if self._state == tkinter.NORMAL:
            ...
            if self._variable is not None:
                ...
                self._variable.set(
                    self._onvalue if self._check_state is True else self._offvalue
                )
                self._variable_callback_blocked = False
