...
def export_portfolio(portfolio: Portfolio, logger=None):
    ...
    try:
        ...
        if result:
            logger.info(
                f"Successfully exported {portfolio.value} portfolio to {result}"
            )
            return True, result
        else:
...
def gui_export():
    ...
    # Create interface
    header = tk.Label(
        root, text="Export Portfolio Workbooks", font=("Helvetica", 14, "bold")
    )
    header.pack(pady=20)
    ...
    alder_btn = tk.Button(
        root,
        text="Export Alder Portfolio to Desktop",
    ...
    wr_btn = tk.Button(
        root,
        text="Export White Rabbit Portfolio to Desktop",
