# app/gui/portfolio_windows/alder.py
...
class AlderPage(BasePage):
    def get_portfolio(self) -> Portfolio:
        return Portfolio.ALDER
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(
            sidebar, text="Workbook Directory", font=("Helvetica", 16, "bold")
        ).grid(row=0, column=0, padx=20, pady=(20, 5), sticky="w")

        # Show current workbook if exists
        current_workbook = self.controller.file_manager.get_portfolio_workbook_path(
            self.portfolio
        )
        ...
        # Upload button
        ctk.CTkButton(
            sidebar, text="Select Workbook", command=self.select_workbook
        ).grid(row=2, column=0, padx=20, pady=(0, 20), sticky="w")

        # Funder Classifier Section
        ctk.CTkLabel(
            sidebar, text="Funder Classifier", font=("Helvetica", 16, "bold")
    ...
    def select_workbook(self):
        ...
        if file_path:
            try:
                ...
                tkmb.showerror(
                    title="Error", message=f"Failed to save workbook: {str(e)}"
                )

    def setup_auto_content(self):
        self.auto_content = DropZone(
            parent=self.content, controller=self.controller, page=self
        )
