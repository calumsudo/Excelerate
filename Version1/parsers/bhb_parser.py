...
def parse_bhb(file_path, output_path, portfolio_name):
    try:
        ...
        # Ensure 'Funder Advance ID' is treated as an integer, except for the totals row
        pivot_table["Funder Advance ID"] = pivot_table["Funder Advance ID"].apply(
            lambda x: x if x == "All" else int(x)
        )
