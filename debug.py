...
class App(ctk.CTk):
    ...
    def authentication_callback(self, success, response, access_token):
        if success:
            ...
            self.show_workbook_ui(get_excel_files(access_token), name)
        else:
            self.authenticate_ui.label.configure(text="Authentication failed.")

    def workbook_callback(self, selected_workbook, output_path, portfolio_name, selected_date):
    ...
    def show_workbook_ui(self, excel_files, user_name):
        self.dashboard_ui = DashboardUI(
            self, excel_files, user_name, self.workbook_callback, self.handle_data_processed
        )
                ...
                if not error:
                    try:
                        final_bytes, detailed_unmatched_info = add_data_to_sheet(
                            self.workbook, pivot_table, sheet_name, self.output_path, self.portfolio_name, self.selected_date
                        )
                        log_to_file(f"Data added to sheet '{sheet_name}'.", self.output_path, self.portfolio_name)
                        updated_content = update_workbook(
                            self.access_token, self.selected_workbook[1], final_bytes, self.output_path, self.portfolio_name
                        )
