...
#         # Rename the columns
#         latest_week_df.columns = ['Funder Advance ID', 'Merchant Name', 'Sum of Syn Gross Amount', 'Sum of Syn Net Amount', 'Total Servicing Fee']

#         # Replace NaN with 0.00 and convert to float
#         latest_week_df = latest_week_df.fillna(0.00)
#         latest_week_df.loc[:, 'Sum of Syn Gross Amount'] = latest_week_df['Sum of Syn Gross Amount'].replace('[\$,]', '', regex=True).astype(float).round(2)
#         latest_week_df.loc[:, 'Sum of Syn Net Amount'] = latest_week_df['Sum of Syn Net Amount'].replace('[\$,]', '', regex=True).astype(float).round(2)
#         latest_week_df.loc[:, 'Total Servicing Fee'] = latest_week_df['Total Servicing Fee'].replace('[\$,]', '', regex=True).astype(float).round(2)
...
def parse_acs(csv_file, output_path, portfolio_name):
    try:
        ...
        # if portfolio_name == "Alder":
        #     df = df.drop([0])  # Drop the row after the header for Alder
        ...
        if not total_row.empty:
            ...
            total_fees = float(str(total_fees).replace("$", "").replace(",", ""))
        else:
