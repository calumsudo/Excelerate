...
def add_net_rtr_column_if_needed(worksheet, header_row=2):
    current_friday = (
        datetime.now() + timedelta((4 - datetime.now().weekday()) % 7)
    ).strftime("%m/%d")
    ...
    if net_rtr_column_index is None:
        ...
        worksheet.cell(row=header_row, column=rtr_balance_column).value = (
            f"Net RTR {current_friday}"
        )
    ...
    for cell in worksheet[header_row]:
        ...
        if cell.value == total_net_rtr_header:
            ...
            for row in range(header_row + 1, worksheet.max_row + 1):
                ...
                worksheet[formula_cell].value = (
                    f"=SUM({start_column_letter}{row}:{end_column_letter}{row})"
                )
...
def map_net_amount_to_excel(worksheet, df_csv, net_rtr_column, header_row=2):
    ...
    for row in worksheet.iter_rows(
        min_row=header_row + 1, min_col=2, max_col=2
    ):  # Adjust min_col and max_col to target Column B
        cell = row[
            0
        ]  # This is the first cell in the row, which corresponds to Column B
        if cell.value:  # Make sure the cell has a value
            merchant_to_row[cell.value.strip()] = (
                cell.row
            )  # Map the merchant name to its row number

    # Now, let's loop through the DataFrame and assign values in Excel
    for index, row in df_csv.iterrows():
        merchant_name = row[
            "Merchant Name"
        ].strip()  # Strip to ensure matching whitespace is not an issue
