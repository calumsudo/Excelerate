...
#                 filename_pattern="ACS_ACG",  # Updated to be more specific
#                 variable_columns=True,
#                 column_patterns={
#                     'base_columns': r'^(Advance ID|Merchant Name|Withdrawn per deal)$',
#                     'week_columns': r'^Week(|\s|\.[0-9]+)$',
#                     'unnamed_columns': r'^Unnamed:\s+\d+'
#                 }
#             ),
...
#                 },
#                 expected_column_count=0,  # Variable
#                 filename_pattern="VESPER CAPITAL",  # Match the exact string
#                 variable_columns=True,
#                 column_patterns={
#                     'base_columns': r'^(Advance ID|Merchant Name|Withdrawn per deal)$',
#                     'week_columns': r'^Week(|\s|\.[0-9]+)$',
...
#         results = {
#             'pattern_matches': 0, 
#             'total_patterns': len(profile.column_patterns),
#             'matched_patterns': set()
#         }
        
#         for col in columns:
#             for pattern_name, pattern in profile.column_patterns.items():
#                 if re.search(pattern, col, re.IGNORECASE):  # Case-insensitive matching
#                     if pattern_name not in results['matched_patterns']:
#                         results['pattern_matches'] += 1
#                         results['matched_patterns'].add(pattern_name)
#                     results[f'{pattern_name}_found'] = True
        
#         # For ACS files, if we see Week columns and unnamed columns, it's a strong indicator
#         if 'week_columns' in results['matched_patterns'] and \
#         'unnamed_columns' in results['matched_patterns'] and \
#         profile.name == "ACS":
#             results['pattern_matches'] += 1  # Bonus point for this combination
...
#         try:
#             # Clean the filename
#             clean_filename = str(file_path.name).strip()
#             print(f"\nDebug - Original filename: '{file_path.name}'")
#             print(f"Debug - Cleaned filename: '{clean_filename}'")
...
#             if last_error:
#                 print(f"Debug - Error reading file: {str(last_error)}")
#                 raise Exception(f"Failed to read file with any encoding. Last error: {str(last_error)}")
#             else:
#                 raise Exception("Failed to read file with any available encoding")
...
#         if profile.name == "Vesper":
#             print("\nDebug Vesper filename matching:")
#             print(f"File name: '{file_info['filename']}'")
#             print(f"File name bytes: {file_info['filename'].encode()}")
#             print(f"Looking for: 'VESPER CAPITAL'")
#             print(f"Contains VESPER? {'VESPER' in file_info['filename']}")
#             print(f"Contains CAPITAL? {'CAPITAL' in file_info['filename']}")
    
        
#         # Filename matching
#         if profile.filename_pattern:
#             # For Vesper, check exact substring
#             if profile.name == "Vesper":
#                 filename_score = 1.0 if "VESPER" in file_info['filename'].strip() else 0.0
#             else:
#                 filename_score = 1.0 if profile.filename_pattern.lower() in file_info['filename'].lower() else 0.0
            
#             filename_weight = 0.5 if profile.name in ["ACS", "Vesper"] else 0.2
...
#         if profile.variable_columns:
#             pattern_results = self.analyze_column_patterns(file_info['columns'], profile)
#             pattern_score = pattern_results['pattern_matches'] / max(1, pattern_results['total_patterns'])
#             pattern_weight = 0.3 if profile.name in ["ACS", "Vesper"] else 0.4
...
#                 'score': 1.0 if file_info['column_count'] > 20 else 0.0,
#                 'weight': 0.2,
#                 'details': f"{'Large' if file_info['column_count'] > 20 else 'Small'} column count ({file_info['column_count']})"
#             }
...
#         # Only for Vesper: if filename doesn't contain VESPER CAPITAL, force total score to 0
#         if profile.name == "Vesper" and "VESPER CAPITAL" not in file_info['filename']:
#             scores['total'] = 0.0
#             scores['components']['override'] = {
#                 'score': 0.0,
#                 'weight': 1.0,
#                 'details': "Forced score to 0 because VESPER CAPITAL not found in filename"
#             }
...
#             if not file_info:
#                 raise Exception("Could not read file information")
...
#         # Simple check for Vesper files
#         if "VESPER" in file_path.name:
#             print(f"\nDebug: Found VESPER in filename: {file_path.name}")
#             return "Vesper"
            
#         # Get funder with highest total score
#         best_funder = max(scores.items(), key=lambda x: x[1]['total'])
#         return best_funder[0] if best_funder[1]['total'] >= threshold else None
...
#                 print("Attempted columns:", file_info.get('columns', 'N/A'))
#                 return
                
#             print(f"\nFile Analysis for: {file_path.name}")
#             print(f"Number of columns: {file_info['column_count']}")
#             print("Columns found:", sorted(file_info['columns']))
